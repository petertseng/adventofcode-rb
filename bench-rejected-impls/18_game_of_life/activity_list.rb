class ActivityList
  def initialize(size, on, diagonals_stuck: false)
    ...
    on.each { |x, y|
      if @cells[y][x] & 1 == 0 && (!@diagonals_stuck || ![x, y].all? { |c| c == 0 || c == @size - 1 })
        @cells[y][x] += 1
  ...
  def step
    ...
    (0...@active_cells).each { |i|
      ...
      if was_alive != is_alive && (!@diagonals_stuck || ![x, y].all? { |c| c == 0 || c == @size - 1 })
    ...
    (0...changed_cells).each { |i|
      ...
      (min_y..max_y).each { |new_y|
        cells_y = @cells[new_y]
        (min_x..max_x).each { |new_x|
          next if @diagonals_stuck && [new_x, new_y].all? { |c| c == 0 || c == @size - 1 }
