...
events_x = ARGF.each_line.with_index.with_object([]) { |(l, i), events|
  captures = COMMAND.match(l).captures
  command = captures.shift.to_sym
  xmin, ymin, xmax, ymax = captures.map(&method(:Integer))
  events << Event.new(xmin, ymin..(ymax + 1), i + 1, command, :begin)
  events << Event.new(xmax + 1, ymin..(ymax + 1), i + 1, command, :end)
}.sort_by { |e| [e.x, e.time] }.freeze
